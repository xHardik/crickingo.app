...
function populatePuzzleBar(puzzleDate) {
  ...
  const launch = new Date('2026-02-01T00:00:00');
  ...
  if (guess === targetPlayer) {
    ...
    if (isInTournament) {
      ...
      showResultCard(
        `🎉 ${targetPlayer}!`,
        currentScore + ' pts',
        `Guessed correctly in ${currentAttempt} ${currentAttempt === 1 ? 'try' : 'tries'}!`
      );
    }
...
function showTournamentEndScreen(score, isWin) {
  document.querySelectorAll('.controls .btn-restart, .controls .btn-back')
...
function showHint(attemptNumber) {
  ...
  if (attemptNumber === 4 && hintPosition) {
...
function buildShareData(score, isWin) {
  ...
  return {
    score:     score + ' pts',
    phrase:    isWin
      ? `Guessed ${targetPlayer} in ${currentAttempt} ${currentAttempt === 1 ? 'try' : 'tries'}!`
      : `Couldn't get ${targetPlayer} today`,
    breakdown: bkd,
...
window.shareScore = async function () {
  ...
  ctx.fillText('CRICKET WORDLE', W / 2, y);
  y += 36;
  ...
  ctx.fillText('📊 Score Breakdown', W / 2, y + 28);
  y += 44;
  ...
  breakdown.forEach(row => {
    ...
    if (row.value) {
      ctx.textAlign = 'right';
      ctx.fillStyle = row.color || '#A855F7';
  ...
  ctx.fillText('Total:', pad + 16, y + 4);
  ctx.textAlign = 'right'; ctx.fillStyle = '#A855F7';
  ctx.fillText(d.score || '0 pts', W - pad - 16, y + 4);
  y += 40;
  ...
  if (tomorrow) {
    ...
    ctx.font      = 'bold 15px Arial'; ctx.fillStyle = '#A855F7';
  ...
  roundRect(ctx, pad, y, W - pad * 2, 58, 12);
  ctx.textAlign = 'center';
  ctx.font      = 'bold 16px Arial'; ctx.fillStyle = '#A855F7';
  ...
  canvas.toBlob(async (blob) => {
    ...
    a.href = url; a.download = 'crickingo-wordle.png';
    document.body.appendChild(a); a.click();
    document.body.removeChild(a); URL.revokeObjectURL(url);
    if (shareCopied) {
      shareCopied.classList.add('show');
      setTimeout(() => shareCopied.classList.remove('show'), 2000);
    }
  }, 'image/png');
};
