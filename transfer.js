...
async function initGame() {
  ...
  if (!currentGame || !currentGame.players) { alert('No transfer history available.'); backToMenu(); return; }
...
function addScoreDisplay() {
  ...
  if (playerBox && !document.getElementById('currentScore')) {
    ...
    scoreDiv.style.cssText = `
      ...
      border: 1px solid rgba(79,142,247,0.3); border-radius: 12px;
    ...
    if (matches.length === 0) {
      ...
      matches.forEach(player => {
        ...
        item.addEventListener('click', () => {
          searchInput.value = player; currentSelectedPlayer = player;
...
function endGame() {
  ...
  if (correctAnswers === currentGame.players.length) { completionBonus = POINTS.COMPLETION; totalScore += completionBonus; }
  ...
  window._shareData = {
    score: finalScore + ' / 1000', phrase: getResultPhrase(correctAnswers).replace(/<[^>]+>/g, ''),
    correct: correctAnswers, total: currentGame.players.length, breakdown: bkd, tomorrow: tomorrowStr
  };
