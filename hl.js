  ...
  try {
    ...
    if (isInTournament) {
      ...
      const sortedKeys    = Object.keys(data)
        .filter(k => data[k].date)
        .sort((a, b) => data[a].date.localeCompare(data[b].date));

      for (const key of sortedKeys) {
        if (data[key].date === requestedDate) {
          selectedDayKey = key;
          selectedDay    = data[key];
          break;
        }
      }
      ...
      if (!selectedDay) {
        for (let i = sortedKeys.length - 1; i >= 0; i--) {
          if (data[sortedKeys[i]].date <= requestedDate) {
            selectedDayKey = sortedKeys[i];
            selectedDay    = data[sortedKeys[i]];
            break;
          }
        }
      }

      if (!selectedDay && sortedKeys.length) {
        selectedDayKey = sortedKeys[sortedKeys.length - 1];
        selectedDay    = data[selectedDayKey];
      }
...
function areComparable(p1, p2) {
  return getStatCategory(p1.stat) === getStatCategory(p2.stat);
}
